...
% generate control run to estimate B
x = x0+dx0;      
contr(:,1) = x;
...
for isample=1:2000
nsamp = obs_freq_timestep;
% ensemble mean
contr_sample=contr(:,(isample-1)*obs_freq_timestep+1:isample*obs_freq_timestep);
zm = mean(contr_sample,2);
Zp = contr_sample - kron(zm,ones(1,nsamp)); 
B(:,:,isample) = Zp*Zp'/(nsamp-1);
end
B=squeeze(mean(B,3));
% save B
save('Static_B.mat','B')